...
function bbp_reply_to_link( $args = array() ) {
	...
	function bbp_get_reply_to_link( $args = array() ) {
		...
		$r['uri'] = $uri;
		$retval   = $r['link_before'] . '<li><a title="Reply for Topic" href="' . esc_url( $r['uri'] ) . '" class="bbp-reply-to-link fa fa-reply"' . $onclick . '></a></li>'. $r['link_after'];
...
function bbp_reply_edit_link( $args = '' ) {
	...
	function bbp_get_reply_edit_link( $args = '' ) {
		...
		$retval = $r['link_before'] . '<li><a title="Edit Topic" href="' . esc_url( $uri ) . '" class="bbp-reply-edit-link fa fa-pencil-square-o"></a></li>' . $r['link_after'];
...
function bbp_reply_trash_link( $args = '' ) {
	...
	function bbp_get_reply_trash_link( $args = '' ) {
		...
		if ( bbp_is_reply_trash( $reply->ID ) ) {
			...
			$actions['trash']   = '<li><a title="' . esc_attr__( 'Move this item to the Trash',      'bbpress' ) . '" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'action' => 'bbp_toggle_reply_trash', 'sub_action' => 'trash',   'reply_id' => $reply->ID ) ), 'trash-'   . $reply->post_type . '_' . $reply->ID ) ) . '" class="bbp-reply-trash-link fa fa-times"></a></li>';
		}
