...
function bbp_topic_edit_link( $args = '' ) {
	...
	function bbp_get_topic_edit_link( $args = '' ) {
		...
		$retval = $r['link_before'] . '<a title="Edit Topic" href="' . esc_url( $uri ) . '" class="bbp-topic-edit-link fa fa-pencil-square-o"></a>' . $r['link_after'];
...
function bbp_topic_trash_link( $args = '' ) {
	...
	function bbp_get_topic_trash_link( $args = '' ) {
		...
		if ( bbp_is_topic_trash( $topic->ID ) ) {
			...
			$actions['trash']   = '<a title="' . esc_attr__( 'Move this item to the Trash',      'bbpress' ) . '" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'action' => 'bbp_toggle_topic_trash', 'sub_action' => 'trash',   'topic_id' => $topic->ID ) ), 'trash-'   . $topic->post_type . '_' . $topic->ID ) ) . '" class="bbp-topic-trash-link fa fa-times"></a>';
		}
...
function bbp_topic_close_link( $args = '' ) {
	...
	function bbp_get_topic_close_link( $args = '' ) {
		...
		$retval  = $r['link_before'] . '<a title="Move this item to Trash" href="' . esc_url( $uri ) . '" class="bbp-topic-close-link fa fa-times"></a>' . $r['link_after'];
...
function bbp_topic_reply_link( $args = array() ) {
	...
	function bbp_get_topic_reply_link( $args = array() ) {
		...
		$r['uri'] = $uri;
		$retval   = $r['link_before'] . '<a title="Reply for this topic" href="' . esc_url( $r['uri'] ) . '" class="bbp-topic-reply-link fa fa-reply"></a>' . $r['link_after'];
